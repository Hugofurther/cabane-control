...
import { X, ChevronLeft, ChevronRight, Droplets, Wind, Thermometer, Calendar } from 'lucide-react';
...
export const WeatherModal = ({ isOpen, onClose, weatherData, initialIndex }) => {
    ...
    useEffect(() => {
        ...
        }, 5000); // Rotate every 5s inside modal
    ...
    const handleContentClick = (e) => {
        e.stopPropagation();
        // If already paused, resume. If running, pause.
    ...
    const handleNext = (e) => {
        e.stopPropagation();
        setCurrentIndex((prev) => (prev + 1) % weatherData.length);
        setIsPaused(true); // Keep paused if manually interacting
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-4 animate-in fade-in" onClick={onClose}>
            ...
            >
                {/* Close Button */}
                <button onClick={onClose} className="absolute top-4 right-4 z-50 p-2 bg-black/40 hover:bg-red-600 rounded-full text-white transition-colors">
                    <X size={20} />
                </button>

                {/* Pause/Manual Indicators */}
                {isPaused && (
                    <>
                        <button onClick={handlePrev} className="absolute left-2 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-blue-600 text-white rounded-full z-50 transition-all">
                            <ChevronLeft size={32} />
                        </button>
                        <button onClick={handleNext} className="absolute right-2 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-blue-600 text-white rounded-full z-50 transition-all">
                            <ChevronRight size={32} />
                        </button>
                        <div className="absolute top-4 left-4 bg-yellow-500/90 text-black text-[10px] font-black px-2 py-1 rounded uppercase tracking-wider z-50">
                            Paused
                        </div>
                    </>
                )}
                ...
                {/* 2. BODY: 5-Day Forecast */}
                <div className="flex-grow overflow-y-auto bg-gray-800 p-4 space-y-3">
                    <h3 className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2 mb-2 sticky top-0 bg-gray-800 py-2 z-10">
                        <Calendar size={14} /> 5-Day Forecast
                    </h3>

                    {Object.entries(dailyForecast).slice(0, 5).map(([day, items]) => {
                        // Get mid-day forecast or max temp logic
                        ...
                        const desc = items[Math.floor(items.length / 2)].weather[0].main;
                        ...
                        return (
                            <div key={day} className="flex items-center justify-between bg-gray-700/50 p-3 rounded-lg border border-gray-600">
                                ...
                                <div className="flex items-center gap-2 flex-grow justify-center">
                                    <img src={`https://openweathermap.org/img/wn/${icon}.png`} className="w-8 h-8" alt="icon" />
                                    <span className="text-xs text-gray-400 font-bold uppercase w-20">{desc}</span>
                                </div>
