...
export const AutomationBanner = () => {
    ...
    if (status.mode === 'SHUTDOWN_ONLY' || status.mode === 'SHUTDOWN_WAIT') {
        ...
        return (
            <div className="fixed bottom-0 left-0 right-0 bg-red-900 border-t-4 border-red-500 shadow-[0_-5px_20px_rgba(220,38,38,0.5)] p-4 z-[100] flex flex-col md:flex-row items-center justify-between gap-4 animate-in slide-in-from-bottom-full">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <div className="font-black text-white uppercase tracking-widest text-lg md:text-xl">
                            {status.mode === 'SHUTDOWN_WAIT' ? "DRAINAGE COMPLETE" : "SYSTEM SHUTDOWN ACTIVE"}
                        </div>
                        {/* ✅ UPDATED TEXT HERE */}
                        <div className="text-sm text-red-200 font-bold uppercase tracking-wide mb-1">
                            Turning all vacuum and valves OFF in:
                        </div>
    ...
    const BranchStatus = ({ id, label, b }) => {
        ...
        if (b.status === 'DONE') { icon = <CheckCircle size={14} className="text-green-500" />; color = "text-green-400 border-green-500/30 bg-green-900/20"; text = "Done"; }
        else if (b.status === 'ERROR') { icon = <AlertTriangle size={14} className="text-red-500" />; color = "text-red-400 border-red-500/30 bg-red-900/20"; text = "Error"; }
        else if (b.timerEnd > 0) {
            const rem = Math.max(0, b.timerEnd - now);
            text = `Wait: ${formatTime(rem)}`;
        }
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-purple-500/50 shadow-2xl p-4 z-[100] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in slide-in-from-bottom-full">
                ...
                <div className="flex flex-col gap-2 w-full">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            {status.shutdownPending && (<span className="text-[10px] bg-red-900/80 text-white border border-red-500 px-2 py-0.5 rounded font-bold flex items-center gap-1"><Power size={10} /> AUTO-OFF</span>)}
                        </div>
                        <span className="text-xs font-mono text-gray-500">{formatTime(totalElapsed)}</span>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 w-full">
                        <BranchStatus id={1} label="Main (ST1-3)" b={status.branches[1]} />
                        <BranchStatus id={2} label="Station 4" b={status.branches[2]} />
                        <BranchStatus id={3} label="Station 5" b={status.branches[3]} />
                    </div>
                </div>
            </div>
