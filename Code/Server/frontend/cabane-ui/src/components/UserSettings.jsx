...
import { X, Save, Volume2, VolumeX, Smartphone, Clock, Layout, Users, Shield, LogOut, Lock, User, Cloud } from 'lucide-react';
...
export const UserSettings = ({ isOpen, onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState('GENERAL'); // ✅ NEW TAB STATE
        ...
        try {
            if (profile.username !== user.username || profile.email !== user.email) {
                await axios.post(`${API_URL}/api/user/profile`,
                    { newUsername: profile.username, newEmail: profile.email },
            ...
            showAlert("Error", e.response?.data?.error || "Failed to save settings.");
        }
    };
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-gray-900 border border-gray-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-700 bg-gray-800 shrink-0">
                    <h2 className="text-xl font-black text-white tracking-wide uppercase flex items-center gap-2"><Users size={20} className="text-blue-500" /> User Settings</h2>
                    <button onClick={onClose} className="text-gray-400 hover:text-white transition-colors"><X size={24} /></button>
                </div>

                {/* TABS */}
                <div className="flex border-b border-gray-700 bg-gray-900 shrink-0">
                    <button onClick={() => setActiveTab('GENERAL')} className={`flex-1 py-3 text-sm font-bold uppercase ${activeTab === 'GENERAL' ? 'text-blue-400 border-t-2 border-blue-500 bg-gray-800' : 'text-gray-500'}`}>General</button>
                    <button onClick={() => setActiveTab('PROFILE')} className={`flex-1 py-3 text-sm font-bold uppercase ${activeTab === 'PROFILE' ? 'text-blue-400 border-t-2 border-blue-500 bg-gray-800' : 'text-gray-500'}`}>Profile & Security</button>
                </div>
                        ...
                        <>
                            <section className="space-y-3">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        <label className="text-[10px] font-bold text-gray-500 mb-1 block">USERNAME</label>
                                        <div className="flex items-center bg-gray-900 border border-gray-600 rounded px-3 py-2">
                                            <User size={16} className="text-gray-500 mr-2" />
                                            <input type="text" value={profile.username} onChange={e => setProfile({ ...profile, username: e.target.value })} className="bg-transparent text-white text-sm outline-none w-full" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-bold text-gray-500 mb-1 block">EMAIL</label>
                                        <input type="email" value={profile.email} onChange={e => setProfile({ ...profile, email: e.target.value })} className="bg-gray-900 border border-gray-600 rounded p-2 text-white text-sm w-full outline-none" />
                                    </div>
                                </div>
                            </section>
