...
import { Shield, LogOut, ScrollText, User, Settings } from 'lucide-react';
    ...
    <div className="min-h-screen bg-cabane-dark text-white p-4 md:p-8 pt-20">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center mb-10 border-b border-gray-700 pb-6 gap-6">
        ...
        <div className="flex gap-3 items-center min-w-[250px] justify-end">
          ...
          {user?.role === 'ADMIN' && (
            <button onClick={() => setShowAdmin(true)} className="p-3 rounded bg-gray-700 hover:bg-blue-900 text-blue-400 hover:text-white transition-colors border border-blue-900/30" title="Admin">
              <Shield size={20} />
            </button>
          )}

          {(user?.can_view_logs || user?.role === 'ADMIN') && (
            <button onClick={() => setShowLogs(true)} className="p-3 rounded bg-gray-700 hover:bg-gray-600 text-yellow-500 transition-colors" title="Logs">
              <ScrollText size={20} />
            </button>
          )}
          ...
          {systemState.currentUser !== user?.username && (
            user?.can_control ? (
              <button onClick={takeControl} className="px-6 py-3 rounded bg-blue-600 hover:bg-blue-500 text-white font-bold uppercase shadow-lg shadow-blue-900/50 transition-all whitespace-nowrap">
                Take Control
              </button>
            ) : (
              <div className="px-4 py-3 rounded bg-gray-800 text-gray-500 font-bold text-xs uppercase border border-gray-700 cursor-not-allowed whitespace-nowrap">
                View Only
              </div>
            )
          )}
          ...
          {systemState.controller === 'USER' && systemState.currentUser === user?.username && (
            <div className="flex gap-2">
              <button onClick={releaseToServer} className="px-4 py-3 rounded bg-yellow-600 hover:bg-yellow-500 text-white font-bold uppercase shadow-lg transition-all whitespace-nowrap">
                Hold
              </button>
              <button onClick={releaseToCabane} className="px-4 py-3 rounded bg-red-600 hover:bg-red-500 text-white font-bold uppercase shadow-lg transition-all whitespace-nowrap">
                Release
              </button>
            </div>
          )}
          ...
          <div className="flex items-center gap-0 bg-gray-800 rounded-lg border border-gray-700 ml-2 overflow-hidden group hover:border-gray-500">
            <button
              ...
              className="px-4 py-3 text-xs text-gray-300 font-bold border-r border-gray-700 flex items-center gap-2 hover:bg-gray-700 hover:text-white transition-colors"
              title="User Settings"
            >
